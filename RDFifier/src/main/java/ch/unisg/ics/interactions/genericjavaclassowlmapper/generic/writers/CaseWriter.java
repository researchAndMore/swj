package ch.unisg.ics.interactions.genericjavaclassowlmapper.generic.writers;
...
import static ch.unisg.ics.interactions.genericjavaclassowlmapper.generic.lists.ListOfSpecialClasses.*;
import static ch.unisg.ics.interactions.genericjavaclassowlmapper.utils.DataUtils.*;
...
public class CaseWriter {
    ...
    private static void iterateFields(Model model, Class<?> clazz, Object instance, Resource objectResource) {
        ...
        // Build the inheritance chain (parent first)
        List<Class<?>> classHierarchy = new ArrayList<>();
        Class<?> currentClass = clazz;

        while (currentClass != null &&
                !currentClass.getName().startsWith("java.") &&
                currentClass != Object.class) {

            classHierarchy.add(0, currentClass); // Add at beginning to get parent classes first
            currentClass = currentClass.getSuperclass();
        }

        // Collect fields in order from parent to child
        for (Class<?> cls : classHierarchy) {
            classFields.addAll(Arrays.asList(cls.getDeclaredFields()));
        }
    ...
    private static void checkTypeAndWriteCorrespondingClass(Resource objectResource, String propertyName, Object fieldInstance, Model model, String namespace, String comparableName, Class<?> fieldType, boolean stop) {
        if (getPrimitiveClassList().contains(comparableName)) {
            writeSimpleLiteral(model, namespace, propertyName, objectResource, model.createTypedLiteral(fieldInstance));
        } else if (getMappingList().containsKey(comparableName)) {
            writeSimpleLiteral(model, namespace, propertyName, objectResource, model.createTypedLiteral(fieldInstance, getMappingList().get(fieldInstance.getClass())));
        } else if (getLanguageClassList().contains(comparableName)) {
